...
class CustomerCubit extends Cubit<CustomerState> {
  ...
  Future<void> joinQueue(int queueId) async {
    try {
      ...
      final currentlyJoined = await _dbHelper.getQueuesByUser(_userId);
      if (currentlyJoined.length >= 3) {
        emit(CustomerError(error: QNowLocalizations.getTranslation('max_queues_reached')));
        return;
      }
      ...
      await loadJoinedQueues(); // Refresh joined queues
      emit(QueueJoined(queueId: queueId));
    } catch (e) {
